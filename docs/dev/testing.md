...
Optional FSDB tests require Verdi. `just prepare-fsdb-fixtures` converts committed VCD fixtures into ignored files under `tests/fixtures/fsdb/` and converts resolved RTL `.fst` artifacts into neighboring `.fsdb` files for benchmark parity. `just test-fsdb` prepares and verifies those generated inputs before running `tests/fsdb_cli.rs` with `--features fsdb`.
