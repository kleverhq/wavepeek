...
Use `extract` commands when you want one row per matching synchronous event with ordered payload values sampled at the pre-edge point. `extract axi` covers AXI3, AXI4, AXI4-Lite, ACE, ACE-Lite, and ACE5 ready/valid channel transfers. `extract generic` covers custom handshakes, FIFO pushes and pops, and other transfer-like rows.
