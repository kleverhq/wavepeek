...
- [x] (2026-05-12 00:00Z) Read the repository maps and planning rules in `AGENTS.md`, `docs/AGENTS.md`, `docs/exec-plans/AGENTS.md`, and the `exec-plan` skill before drafting.
- [x] (2026-05-12 00:00Z) Mapped the current shipped skill surface across `src/cli/docs.rs`, `src/cli/mod.rs`, `src/engine/docs.rs`, `src/engine/mod.rs`, `src/docs/mod.rs`, `docs/public/commands/docs.md`, `docs/public/commands/overview.md`, `docs/public/intro.md`, `README.md`, `docs/DEVELOPMENT.md`, `docs/skills/AGENTS.md`, `tests/docs_cli.rs`, and `tests/cli_contract.rs`.
- [x] (2026-05-12 00:00Z) Searched live repository collateral for `docs skill`, `packaged skill`, `agent skill`, and related phrases to separate required collateral from no-change surfaces.
- [ ] Create the initial plan commit for this new active plan directory.
- [ ] Run focused read-only subagent review lanes on the plan.
- [ ] Revise this plan from review findings, record the findings in the living sections, and create the reviewed plan commit.
- [ ] Implementation has not started. No source, docs, or test collateral outside this plan file should change during this planning task.
...
This plan is not implemented yet. The current outcome is only repository research and a reviewed implementation plan. The branch result for this request should be two commits: one initial plan checkpoint and one reviewed plan revision after read-only subagent feedback.
