...
This plan targets exactly one roadmap boundary: `C3` from `docs/expression_roadmap.md`. `docs/expression_roadmap.md` is the authoritative phase contract when it is more specific than version-oriented bullets in `docs/ROADMAP.md`. Earlier roadmap wording was less explicit about the `C3` versus command-integration split; current wording now aligns with `C3` standalone scope and `C5` command rollout. This plan follows that phase contract and still keeps public command rollout deferred to `C5`.
