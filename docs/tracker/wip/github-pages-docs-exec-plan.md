...
After this change, maintainers can build and check a human-friendly web version of the same public documentation that ships inside the `wavepeek` CLI. A release publication will add an immutable documentation snapshot such as `/0.5.0/`, move `/latest/` to the newest stable release, redirect the site root to `/latest/`, and expose raw root schema artifacts at `/wavepeek_vX.json`.

The behavior is observable without publishing anything: from the repository root inside the wavepeek devcontainer, `just docs-site-check` prepares a generated MkDocs staging tree under `tmp/docs-site/`, builds the site in strict mode, and prepares raw schema root artifacts. Publication uses MkDocs Material for the site and `mike` to manage the cumulative `gh-pages` branch archive.
...
- [x] (2026-06-08 07:22Z) Milestone 3 implemented and locally validated. Added root `mkdocs.yml`, `tools/docs/prepare_mkdocs.py`, and `tools/docs/test_prepare_mkdocs.py`. Validation passed for `cargo run --quiet -- docs export tmp/docs-site/export --force`, `python3 -B tools/docs/prepare_mkdocs.py tmp/docs-site/export --output tmp/docs-site/mkdocs-src --config-output tmp/docs-site/mkdocs.yml --version 0.5.0 --force`, `mkdocs build --strict --config-file tmp/docs-site/mkdocs.yml`, and `python3 -B -m unittest discover -s tools/docs -p "test_*.py"` with the temporary docs tooling venv on `PATH`.
- [x] (2026-06-08 07:28Z) Milestone 4 implemented and locally validated. Added `tools/docs/publish_docs.py` and `tools/docs/test_publish_docs.py` with `check`, `stage-deploy`, and `push-staged` subcommands. The wrapper validates SemVer/source-tag agreement, runs old or current source exports through trusted current tooling, prepares root schema artifacts, stages mike deploys without push-token environment variables, bundles staged `gh-pages`, and verifies staged bundles before non-forced push. Validation passed for `python3 -B -m unittest discover -s tools/docs -p "test_*.py"`, `python3 -m py_compile tools/docs/prepare_mkdocs.py tools/docs/publish_docs.py tools/docs/test_prepare_mkdocs.py tools/docs/test_publish_docs.py`, `python3 -B tools/docs/publish_docs.py check --version 0.5.0 --source-root . --work-dir tmp/docs-site-current`, and `python3 -B tools/docs/publish_docs.py check --version 0.5.0 --source-ref v0.5.0 --work-dir tmp/docs-site-v050` with the temporary docs tooling venv on `PATH`.
- [x] (2026-06-08 07:30Z) Milestone 5 implemented and locally validated. Added `docs-site-build`, `docs-site-serve`, `docs-site-check`, `docs-site-stage-deploy`, `docs-site-push-staged`, and `docs-site-deploy` recipes; included `tools/docs` unit tests in `test-aux`; and wired `docs-site-check` into both `check` and `ci`. Validation passed for `just --unstable --fmt --check`, `just --summary`, `just docs-site-build`, `just docs-site-check`, and `just test-aux` with the temporary docs tooling venv on `PATH`.
...
- [x] (2026-06-08 08:15Z) Targeted verifier recheck reported no substantive findings. Final gates passed: `just check` and `just ci` with `PATH="$PWD/tmp/docs-tooling-venv/bin:$PATH"` in the current container. The full implementation is complete except for repository-side GitHub Pages settings and the actual post-merge/manual publication run.
- [x] (2026-06-08 12:47Z) User changed publication policy: envelope `$schema` URLs should use GitHub Pages, and the packaged skill should not be published as `/skill.md`. Implementation updates changed `SCHEMA_URL`, schema artifacts and schema contract tests, public examples, and docs publication helpers so only `wavepeek_v*.json` schema files are prepared and verified as Pages root artifacts. Validation passed for `just check-schema`, `cargo test -q --test schema_cli`, `python3 -B -m unittest discover -s tools/docs -p "test_*.py"`, `just docs-site-check`, and `python3 -B tools/docs/publish_docs.py check --version 0.5.0 --source-ref v0.5.0 --work-dir tmp/docs-site-v050`.
- [x] (2026-06-08 12:55Z) Focused review for the schema URL and skill publication policy change completed. The docs reviewer reported no substantive findings. The implementation keeps the policy simple by never collecting, allowing, deleting, or verifying `/skill.md` as a Pages artifact. Validation passed for `python3 -B -m unittest discover -s tools/docs -p "test_*.py"`, `python3 -B tools/docs/publish_docs.py check --version 0.5.0 --source-ref v0.5.0 --work-dir tmp/docs-site-v050`, `just docs-site-check`, and `git diff --check`.
- [x] (2026-06-08 19:30Z) PR review identified that a `GITHUB_TOKEN` push to `gh-pages` may not trigger a branch-based GitHub Pages build. The implementation now keeps `gh-pages` as the `mike` state store, exports the verified staged tree to `tmp/docs-site/pages-artifact/` during `push-staged`, and deploys that artifact through `actions/upload-pages-artifact@v4` plus `actions/deploy-pages@v4`. Initial validation passed for `python3 -B -m unittest tools/docs/test_publish_docs.py`, `python3 -B -m unittest discover -s tools/docs -p "test_*.py"`, `actionlint .github/workflows/*.yml`, `just docs-site-check`, and the historical `v0.5.0` check.
- [x] (2026-06-08 19:42Z) Focused helper review found that `git archive` can honor `.gitattributes export-ignore` in the staged tree and mutate the Pages artifact after verification. The export implementation now uses a detached worktree plus file copy instead of `git archive`, and tests cover an `export-ignore` entry that must not remove a verified page from `tmp/docs-site/pages-artifact/`. Validation passed for `python3 -m py_compile tools/docs/publish_docs.py tools/docs/test_publish_docs.py`, `python3 -B -m unittest tools/docs/test_publish_docs.py`, `GIT_INDEX_FILE=/tmp/nonexistent-index python3 -B -m unittest tools/docs/test_publish_docs.py`, `python3 -B -m unittest discover -s tools/docs -p "test_*.py"`, `actionlint .github/workflows/*.yml`, and `git diff --check`.
- [x] (2026-06-08 19:52Z) Targeted recheck for the detached-worktree Pages artifact export reported no substantive findings. Final local gates for this change passed with `PATH="$PWD/tmp/docs-tooling-venv/bin:$PATH" just test-aux` and `PATH="$PWD/tmp/docs-tooling-venv/bin:$PATH" just check`.
...
- Observation: At the start of implementation, `docs/public/` authored front matter with `summary`, while the desired authored and runtime field was `description`. The current main branch already contained breaking changes and was expected to release as a future major version, so this plan renamed the current docs metadata field end-to-end instead of preserving `summary` in new JSON output.
  Evidence: before Milestone 1, `docs/public/intro.md` began with `summary: Start here...`; `src/docs/mod.rs` had `FrontMatter { summary: String }` and `TopicSummary { summary: String }`; `tests/docs_cli.rs` asserted that JSON topic entries contained `summary`; the user explicitly requested the full rename on 2026-06-08.
...
- Observation: Pushing `gh-pages` from a workflow with `GITHUB_TOKEN` is not a reliable trigger for a branch-based GitHub Pages build.
  Evidence: PR review on 2026-06-08 cited GitHub's documented behavior that commits pushed by workflows using `GITHUB_TOKEN` do not trigger new workflow runs, including branch-triggered Pages builds. The fix keeps `gh-pages` as canonical `mike` state, but publishes the verified tree through GitHub Pages Actions.

- Observation: `git archive` is not a safe way to materialize the verified Pages tree because it honors `.gitattributes` from that tree.
  Evidence: focused helper review on 2026-06-08 noted that allowed paths such as `0.5.0/.gitattributes` could mark verified files with `export-ignore` or `export-subst`, causing `tmp/docs-site/pages-artifact/` to differ from the verified commit. The fix exports via detached worktree copy instead.

- Observation: Git helper tests and publication helpers can inherit `GIT_INDEX_FILE` from pre-commit hook execution, which breaks temporary repository commits and worktree operations.
  Evidence: the first commit attempt after adding detached worktree export failed in `just test-aux` with `fatal: .git/index: index file open failed: Not a directory`. The fix removes `GIT_INDEX_FILE` from helper subprocess environments and from the test Git helper.
...
- Decision: Use GitHub Pages schema URLs in JSON envelopes, such as `https://kleverhq.github.io/wavepeek/wavepeek_v0.json`, rather than raw GitHub URLs for schema artifacts on `main`.
  Rationale: Pages is now the public documentation and schema publication surface, while raw `main` points at a moving source branch rather than the released docs archive. Major schema filenames remain stable root artifacts, so the URL does not need a per-version directory.
  Date/Author: 2026-06-08 / Grin, confirmed by user after implementation review.

- Decision: Publish all files matching `schema/wavepeek_v*.json` from the release source checkout to the site root, but do not publish `docs/skills/wavepeek.md` as `/skill.md`. For the legacy `v0.5.0` source only, if no `wavepeek_v*.json` files exist and `schema/wavepeek.json` exists for package major version `0`, copy it to the site root as `wavepeek_v0.json`.
  Rationale: The root schema artifacts must represent the latest stable release context, and the user explicitly requested all major schema versions rather than a hard-coded single schema. The packaged skill must stay coupled to the installed CLI through `wavepeek skill`; publishing a root `skill.md` would make it easy for agents to fetch an incompatible latest skill for an older binary. The existing `v0.5.0` tag predates the versioned schema filename, so the first bootstrap needs a narrow compatibility mapping to satisfy the required root schema URL without moving the historical tag.
  Date/Author: 2026-06-08 / Grin, updated by user after initial implementation and review.
...
- Decision: Split publication into a no-token staging step and a credentialed push/deploy step.
  Rationale: The staging step compiles and runs code from the selected release tag through `cargo run`. Push and Pages deployment credentials must not be present in that process tree. A separate push/deploy step can receive credentials after all release-source build/check work is complete.
  Date/Author: 2026-06-07 / Grin, from second control review; updated 2026-06-08 for explicit Pages Actions deployment.
...
- Decision: Run the credentialed push and deploy job from a fresh trusted checkout using only uploaded staging artifacts from the no-token staging job.
  Rationale: Release-source build code runs during staging and has workspace write access. The token-bearing job must not trust scripts, just recipes, `.git` state, runner state, or metadata from that same mutable workspace except as uploaded data to verify and import.
  Date/Author: 2026-06-07 / Grin, from third and fourth control reviews; updated 2026-06-08 for explicit Pages Actions deployment.

- Decision: Verify staged `gh-pages` bundles against remote base state, version metadata semantics, and an allowed path set before pushing, then deploy the verified tree with GitHub Pages Actions.
  Rationale: The deploy job receives generated data from a prior job. It must prove the bundle is a fast-forward update from the remote base observed during staging, does not rewrite an existing requested version unless an explicit repair mode is present, preserves unrelated version entries and snapshots, only adds the requested version while moving `latest` and the default to it, and changes only allowed paths. Because a `GITHUB_TOKEN` push to `gh-pages` should not be relied on to trigger Pages, the verified tree is exported as an Actions Pages artifact and deployed with `actions/deploy-pages`.
  Date/Author: 2026-06-07 / Grin, from fourth and fifth control reviews; updated 2026-06-08 from PR review.
...
- Decision: Start automatic docs publication only after the release workflow has successfully created the GitHub Release, not directly from tag push.
  Rationale: Tag push currently starts the binary release workflow. Starting docs from the same tag event creates independent deployments that can race: docs can publish before the binary release succeeds, or docs can fail while the binary release succeeds. A created GitHub Release is the durable signal that the crate release and user-facing release have completed. Because GitHub Releases created with the default `GITHUB_TOKEN` do not fire downstream `release` workflows, the release workflow should explicitly dispatch the trusted docs workflow after creating the GitHub Release. Manual docs dispatch must also verify default-branch tooling and a published stable release before staging Pages output.
  Date/Author: 2026-06-08 / Grin, confirmed by user, release/docs trigger review, and implementation review.
...
- Decision: Keep the release workflow recoverable enough that GitHub Release creation can be retried after a partial crate publish.
  Rationale: Docs publication depends on the GitHub Release existing and on the release workflow dispatching docs after creating it. If `cargo publish` succeeds but GitHub Release creation fails, maintainers need a safe rerun path that verifies the crate version is already on crates.io and then creates the missing GitHub Release, rather than moving the tag or minting a new release tag. The implemented tag workflow checks the crates.io version endpoint and skips `cargo publish` when the crate version already exists, so a failed later step can be rerun without hitting a duplicate publish error. If the original tag-push workflow definition or helper scripts were broken, recovery must use the trusted current docs workflow manual dispatch after the stable GitHub Release exists.
  Date/Author: 2026-06-08 / Grin, from release/docs trigger review and implementation review.

## Outcomes & Retrospective

All implementation milestones are complete. The current docs metadata contract now uses `description` in authored Markdown, current CLI JSON, export manifests, schema, docs, and tests, with `summary` retained as a legacy front matter input alias. Envelope `$schema` URLs now point to the GitHub Pages schema artifact path such as `https://kleverhq.github.io/wavepeek/wavepeek_v0.json`. The devcontainer source installs MkDocs and mike from root `requirements-docs.txt` into the shared image and verifies both in `just dev-setup`. The MkDocs preparation helper exports embedded docs, stages `intro` as `index.md`, normalizes legacy `summary` front matter to `description` only in generated staging files, generates nav from the export manifest, and builds in MkDocs strict mode. The publication wrapper checks current or release-tag source, creates root schema artifacts only, stages a mike deploy, bundles `gh-pages`, verifies the staged bundle, exports the verified tree to `tmp/docs-site/pages-artifact/` through detached worktree copy, and then pushes `gh-pages` as the canonical `mike` state branch. Push-side verification recomputes trusted path rules, checks root schema artifacts as blobs, rejects malformed `versions.json`, and preserves unrelated versions. The packaged skill is not published to Pages; `wavepeek skill` remains the supported way to get a skill matched to the installed CLI. Just recipes expose build, serve, check, staged deploy, push-staged, combined deploy, and remote workflow dispatch flows; docs-site checks run in both local and CI quality gates. GitHub Actions now has a manual docs publication workflow with split staging and push/deploy jobs, and the release workflow dispatches docs publication only after creating the GitHub Release. The push/deploy job keeps `gh-pages` updated but deploys the verified tree through GitHub Pages Actions instead of relying on a branch-push-triggered Pages build. Reviews ran in code, workflow/release, docs, control, targeted recheck, and policy-change lanes. Validation passed for all focused commands recorded in Progress, plus final `just check` and `just ci` with `PATH="$PWD/tmp/docs-tooling-venv/bin:$PATH"` before the schema URL and Pages Actions policy changes; after the Pages Actions change, `PATH="$PWD/tmp/docs-tooling-venv/bin:$PATH" just test-aux` and `PATH="$PWD/tmp/docs-tooling-venv/bin:$PATH" just check` passed, and targeted recheck reported no substantive findings. The historical `v0.5.0` bootstrap was dry-run locally with `python3 -B tools/docs/publish_docs.py check --version 0.5.0 --source-ref v0.5.0 --work-dir tmp/docs-site-v050` and now prepares only `wavepeek_v0.json` as a root artifact. Remaining work outside the branch is repository configuration and execution: after merge, confirm GitHub Pages is configured for GitHub Actions deployments, then use `.github/workflows/docs.yml` manual dispatch for the initial `0.5.0` bootstrap or wait for future releases to trigger docs publication through `.github/workflows/release.yml`.
...
The embedded docs loader is in `src/docs/mod.rs`. It loads Markdown files from `docs/public/`, parses YAML front matter, validates that each file path matches its stable topic ID, validates `see_also` references, and exposes `TopicSummary` values with `id`, `title`, `description`, `section`, and `see_also`. Current runtime/export output uses `description`; legacy `summary` is accepted only at input boundaries for old Markdown and historical manifests. The export path writes topic Markdown files and a `manifest.json` into an output directory. The manifest is the correct contract for site generation because it describes the exact docs embedded in that build.

The CLI docs behavior is tested in `tests/docs_cli.rs`, and additional runtime edge tests for `src/docs/mod.rs` live in `src/tests/docs_runtime_edges.rs`. The docs CLI includes a `docs show --description` option in `src/cli/docs.rs`; the former `--summary` option is intentionally unsupported. Public docs style conventions currently live in `docs/dev/style.md`, which describes `description` front matter and notes the legacy `summary` input alias.

The packaged agent skill source is `docs/skills/wavepeek.md`, but it is intentionally not published to Pages. The canonical machine-output schema artifacts live under `schema/`, currently `schema/wavepeek_v0.json` on the working branch. The docs JSON output is covered by that schema, so the `summary` to `description` rename and the Pages schema URL must update the schema artifact as well as code and tests. The current `wavepeek schema` command emits the checked-in schema artifact through `include_str!`, so implementation must update `schema/wavepeek_v0.json` directly or add a real generator before running `just check-schema`; `just update-schema` alone cannot derive the rename from Rust structs. The historical `v0.5.0` tag instead has `schema/wavepeek.json`; publication must normalize that legacy major-0 artifact to the root filename `wavepeek_v0.json`. Schema files are not part of `wavepeek docs export`, but the web publication must expose them as raw static files at the site root.
...
GitHub Actions workflows currently live in `.github/workflows/ci.yml` and `.github/workflows/release.yml`. They run in the devcontainer baseline through `devcontainers/ci`. The existing release workflow is triggered by `push` tags matching `v*.*.*`; it publishes the crate and creates the GitHub Release. The new docs publication workflow should be separate, should grant write permission only to the push/deploy job that updates `gh-pages` and deploys through GitHub Pages Actions, should be dispatched automatically by the release workflow only after the GitHub Release exists, and should also support manual dispatch for the initial historical `v0.5.0` bootstrap and docs repair.
...
Raw artifacts are static files served directly as JSON, not rendered as HTML pages. In this project they are root schema files such as `/wavepeek_vX.json`.

The `gh-pages` branch is the dedicated versioned state branch managed by `mike`. It contains generated HTML and raw artifacts, not source docs. The workflow also exports the verified `gh-pages` tree as a GitHub Pages Actions artifact for actual publication.

## Open Questions

There are no blocking product questions remaining from the user. One repository setting remains outside source control: after the workflow exists, a maintainer must ensure GitHub Pages is configured for GitHub Actions deployments.
...
At the end of this milestone, one helper can build from the current checkout or from a release ref such as `v0.5.0`, deploy a version snapshot with `mike`, set `latest`, set the root default redirect, and update raw root schema artifacts without rebuilding old snapshots.
...
Create `tools/docs/publish_docs.py`. It should be deterministic, non-interactive, and safe to run in CI. It should provide at least these subcommands:
    ...
    check: export docs, prepare MkDocs staging, run mkdocs build strict, and prepare root schema artifacts under tmp/docs-site/root-artifacts without touching gh-pages.
    stage-deploy: perform the check work, run mike deployment for a version against the local gh-pages branch, set latest/default aliases, commit root schema artifacts to the local gh-pages branch, write a small stage metadata file under tmp/docs-site/, and create a git bundle or equivalent data artifact containing only the staged gh-pages branch state.
    push-staged: from a fresh trusted checkout in a separate push/deploy job, verify the stage metadata and bundle, verify the update against the current remote gh-pages state and allowed path set, import the staged gh-pages branch state, export the verified tree to tmp/docs-site/pages-artifact, and push the state branch to origin. This subcommand must not run cargo, mkdocs, mike, or any command that uses SOURCE_REF.
...
Acceptance for this milestone is that both check commands build a strict site, prepare at least `tmp/docs-site/root-artifacts/wavepeek_v0.json`, do not prepare `tmp/docs-site/root-artifacts/skill.md`, and do not touch the remote repository.
...
This runs the same strict build as `docs-site-build`, then prepares root schema artifacts from the current checkout and verifies that at least one `wavepeek_v*.json` exists under `tmp/docs-site/root-artifacts/` and `skill.md` is absent.
...
Set workflow permissions at the workflow or job level so the staging job has only what it needs for checkout, artifact upload, and optional GHCR cache access, while the push/deploy job is the only job with `contents: write` for pushing `gh-pages`, `pages: write` for Pages deployment, and `id-token: write` for `actions/deploy-pages`. Add `packages: write` only to jobs that write the GHCR devcontainer cache; if the docs workflow deliberately uses cache read-only mode, document that choice and omit `packages: write`. Add a concurrency group such as `docs-pages` so two docs publications cannot push over each other.
...
After that devcontainer step succeeds, upload the staged deploy metadata and `gh-pages` bundle as a workflow artifact. Add a separate push/deploy job, not merely a later step in the same job. The push/deploy job must start from a fresh clean checkout of the trusted workflow ref, not from the staging workspace, and must download only the staged data artifact from the staging job. That job may receive `GITHUB_TOKEN`, should verify `staged-deploy.json`, verify the bundle ref and expected final commit, fetch current `origin/gh-pages`, verify that the remote base commit still matches the metadata or that the metadata explicitly marks first publish, verify the staged commit is a fast-forward update, reject the push if requested `VERSION` already exists in the remote base unless the explicit repair input is true and the staged metadata also records repair mode, compare remote-base and staged `versions.json` so unrelated version entries and aliases are preserved, verify the only semantic version metadata changes are adding or repairing `VERSION` and moving `latest` plus the default to it, verify that changed paths are limited to the requested version directory, `latest` alias/default files, mike metadata such as `versions.json`, `.nojekyll`, and root `wavepeek_v*.json`, export the verified staged tree to `tmp/docs-site/pages-artifact/`, and then push the `gh-pages` state branch without force. It may run either a second devcontainer invocation that calls:
...
After `push-staged` succeeds, the workflow must upload `tmp/docs-site/pages-artifact/` with `actions/upload-pages-artifact` and deploy it with `actions/deploy-pages`. Set the job environment to `github-pages` and expose the deployment URL from the deploy step. The Pages deploy step is the actual site publication; the `gh-pages` push is retained as canonical versioned state for `mike`, not as the trigger for a branch-based Pages build.

This push/deploy job must not run cargo, mkdocs, mike, any script from the staging workspace, or any command that checks out or executes `SOURCE_REF`.
...
Acceptance for this milestone is that `actionlint` passes, any release recovery helper tests pass, the docs workflow has no tag-push trigger, the release workflow dispatches the docs workflow only after creating the GitHub Release, the docs workflow preflights that the GitHub Release exists and is stable, trusted checkouts use explicit refs rather than accidentally checking out the release tag, the push/deploy job grants `contents: write`, `pages: write`, and `id-token: write`, the verified staged tree is uploaded with `actions/upload-pages-artifact` and deployed with `actions/deploy-pages`, the release recovery path can create a missing GitHub Release from trusted current tooling after verifying crates.io already has the version and then dispatch docs, and the workflow clearly shows how to publish `0.5.0` or repair a failed docs publication from manual dispatch without requiring the old tag to contain `.github/workflows/docs.yml`.
...
The `tmp/docs-site/root-artifacts/skill.md` path must not exist; the packaged skill remains available through `wavepeek skill` for the installed CLI.
...
That command must export docs by compiling the release source from `v0.5.0`, build a strict site using the current scripts, and prepare root schema artifacts from the `v0.5.0` source checkout without preparing `skill.md`.
...
Expected generated local layout after `just docs-site-check`:
    ...
    tmp/docs-site/root-artifacts/  generated wavepeek_v*.json root schema files
...
`tools/docs/publish_docs.py` must expose `check`, `stage-deploy`, and `push-staged` subcommands. `check` accepts `--version`, `--source-root`, and optionally `--source-ref`. `stage-deploy` requires `--version` and a non-empty SemVer `--source-ref`, accepts `--repair-existing-version` only when explicitly requested, and writes metadata plus a staged `gh-pages` bundle under the configured work directory. `push-staged` accepts `--version`, `--work-dir`, and optional `--repair-existing-version`, validates the metadata and bundle from that work directory, imports the staged `gh-pages` state into the fresh checkout, fetches current `origin/gh-pages`, verifies remote base and fast-forward ancestry, rejects existing requested versions unless repair mode is explicit, verifies allowed `versions.json` semantic changes, checks changed paths against the allowed set, verifies required root schema blobs, and pushes without force. Subprocess invocation must stay in small functions so tests can mock command execution.
...
2026-06-08 07:22Z: Recorded Milestone 3 implementation. The generated MkDocs staging helper now validates export manifests, normalizes metadata for staged Markdown, writes generated navigation/config under `tmp/docs-site/`, and passes strict MkDocs build validation.

2026-06-08 07:28Z: Recorded Milestone 4 implementation. The publication wrapper now separates local check, local staged deploy, and push-only verification paths; it also supports rebuilding historical release docs from `v0.5.0` through current trusted tooling.

2026-06-08 07:30Z: Recorded Milestone 5 implementation. The root `justfile` now exposes docs-site build/check/deploy recipes, includes docs helper tests in `test-aux`, and gates `check`/`ci` on `docs-site-check`.
...
2026-06-08 08:15Z: Recorded completion. The targeted verifier recheck reported no substantive findings, and final `just check` plus `just ci` passed in the current container with the temporary docs tooling venv on `PATH`.

2026-06-08 12:47Z: Recorded schema URL and skill publication policy change. Envelope `$schema` URLs now target GitHub Pages root schema artifacts, and `/skill.md` is no longer prepared or verified as a Pages publication artifact.

2026-06-08 12:55Z: Recorded focused review outcome for `/skill.md`. The implementation does not include cleanup or verification for that path; `/skill.md` is simply outside the Pages publication surface.

2026-06-08 19:30Z: Recorded PR review finding about branch-triggered Pages builds from `GITHUB_TOKEN` pushes. The docs workflow now deploys the verified staged tree with GitHub Pages Actions while keeping `gh-pages` as `mike` state.

2026-06-08 19:42Z: Recorded helper review fix for Pages artifact export. The helper now materializes the verified tree through a detached worktree copy instead of `git archive`, so `.gitattributes` cannot alter the deploy artifact after verification. Helper subprocesses now also clear `GIT_INDEX_FILE` so pre-commit hook environments do not corrupt temporary Git repositories.

2026-06-08 19:52Z: Recorded targeted recheck and final local gates for the Pages Actions deploy change. The recheck reported no substantive findings, and `just test-aux` plus `just check` passed.
