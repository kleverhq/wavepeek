...
if ($env:WAVEPEEK_DOWNLOAD_URL) {
  ...
  $ArtifactDownloadUrls = @("$installer_base_url/kleverhq/wavepeek/releases/download/v3.0.1")
} elseif ($env:WAVEPEEK_INSTALLER_GITHUB_BASE_URL) {
  $installer_base_url = $env:WAVEPEEK_INSTALLER_GITHUB_BASE_URL
  $ArtifactDownloadUrls = @("$installer_base_url/kleverhq/wavepeek/releases/download/v3.0.1")
} else {
  $ArtifactDownloadUrls = @("https://github.com/kleverhq/wavepeek/releases/download/v3.0.1")
}
