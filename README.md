...
wavepeek extract generic --waves "$WAVES" --from 0ns --to 500ns --scope top.cpu --on 'posedge clk' --when 'valid && ready' --payload data,last
```
...
| `extract generic` | Protocol-neutral event rows with pre-edge payload values |
...
- `wavepeek schema` for packaged JSON output contracts, `wavepeek schema --stream` for JSONL, and `wavepeek schema --input` for structured input
