...
- Observation: FSDB files produced by `fst2vcd` followed by `vcd2fsdb` do not currently provide byte-for-byte hierarchy parity with the committed FST benchmark baseline for SCR1 `scope` and recursive `signal` benchmark cases.
  Evidence: an attempted FSDB smoke including `scope_scr1_all_depth7_json` and `signal_scr1_top_recursive_depth2_json` failed `compare --functional-only --allow-golden-extra` with `mismatched fields data` for those cases, while `info_picorv32_ez`, `value_scr1_signals_1`, and `change_scr1_signals_1_window_2ns_trigger_any` passed. The Make smoke now gates the parity-stable `info`/`value`/`change` subset; FSDB-specific integration tests continue covering `scope` and `signal` behavior on controlled generated fixtures.
...
- Decision: keep the committed `bench-e2e-fsdb-smoke-commit` target strict but narrow it to FSDB-vs-FST parity-stable `info`, `value`, and `change` benchmark cases.
  Rationale: converted SCR1 FSDB hierarchy-listing payloads currently differ from the existing FST baseline for `scope` and recursive `signal`. Weakening functional-only compare would hide real breakage, while keeping known false failures would make the smoke useless. Controlled FSDB integration fixtures still cover `scope` and `signal`; the benchmark smoke gates the large-artifact command payloads that are currently stable.
  Date/Author: 2026-05-30 / Grin

## Outcomes & Retrospective

Milestones 1 through 5 and the first focused review loop are implemented. The repository now has a committed FSDB benchmark catalog, fixture preparation that can convert both hand-authored VCD fixtures and external RTL FST artifacts, FSDB benchmark Make targets, a writable devcontainer artifact-copy fix for future rebuilt images, catalog path remapping for nonstandard artifact roots, and functional-only FSDB-vs-FST compare behavior that treats timeout artifacts as failures. The FSDB runtime now uses a command-scoped native signal session with bounded lazy traversal-handle reuse for sampling/event queries and temporary handles for broad candidate sweeps; safe Rust ties the session lifetime to the reader through `Rc`, and the backend reopens sessions atomically as commands request additional idcodes. Review-driven fixes hardened caller-owned native session close behavior, cached handle exception safety, parallel Make target ordering, artifact-dir fallback resolution, and stale parity documentation. Documentation and changelog now describe the maintainer-visible fixture/benchmark/session changes. Validation so far: `python3 -B -m unittest discover -s bench/e2e -p 'test_*.py'`, `python3 -m py_compile bench/e2e/perf.py bench/e2e/test_perf.py scripts/check_fsdb_bench_artifacts.py`, `WAVEPEEK_IN_CONTAINER=1 make test-aux`, `make -n check-fsdb-rtl-artifacts`, a direct fixture-preparation run against a writable temporary RTL artifact directory, `cargo test --features fsdb --test fsdb_cli`, FSDB native smoke tests, `cargo clippy --features fsdb --all-targets -- -D warnings` under `CARGO_TARGET_DIR=target/fsdb`, `make check`, `make ci`, `make test-fsdb` with `RTL_ARTIFACTS_DIR=$PWD/tmp/rtl-artifacts-fsdb-smoke`, and `make -j4 bench-e2e-fsdb-smoke-commit` with the same writable artifact override. The remaining work is a final independent control review, any control-review fixes, and handoff.
...
`bench-e2e-fsdb-smoke-commit` must be a real gate, not just a timing capture with a hopeful smile. The implemented gate runs the parity-stable `info_picorv32_ez`, `value_scr1_signals_1`, and `change_scr1_signals_1_window_2ns_trigger_any` subset into a temporary directory and then calls `bench/e2e/perf.py compare --functional-only --allow-golden-extra --revised "$$tmp_revised" --golden "$(BENCH_E2E_BASELINE_DIR)"`. Earlier drafts targeted one `scope` and one `signal` benchmark too, but the 2026-05-30 discovery recorded below shows converted SCR1 FSDB hierarchy payloads do not currently match the committed FST baseline byte-for-byte. The `--allow-golden-extra` option is only for filtered smoke runs where the golden baseline intentionally contains tests that were not run in the revised directory. Full FSDB benchmark comparisons must not use it.
...
completes without functional mismatches, timeout artifacts, or missing revised artifacts for the parity-stable smoke subset. A full local FSDB run may still be useful for timing evidence, but it is not currently accepted as a strict FSDB-vs-FST functional gate because converted SCR1 `scope` and recursive `signal` hierarchy payloads are known to differ from the committed FST baseline. When maintainers investigate or fix those hierarchy differences, they should rerun strict full `compare --functional-only` and update this plan or a follow-up plan with the result. The implementation should improve or hold steady against a pre-session FSDB baseline on repeated-sampling `change` and `property` cases, allowing at most 15% median regression unless this plan records a measured exception.
...
- 2026-05-30 / Grin: Completed Milestones 4 and 5 implementation notes after updating docs/changelog and running default plus FSDB gates. Recorded the converted SCR1 hierarchy parity surprise and narrowed the FSDB benchmark smoke to strict parity-stable `info`/`value`/`change` cases instead of weakening functional-only compare semantics.
- 2026-05-30 / Grin: Recorded focused review findings and fixes. Native close/tombstone behavior, exception-safe cached handle insertion, parallel Make ordering, artifact helper fallback resolution, and stale full-parity plan text were corrected before the final control review.
