...
- [ ] Implement FSDB benchmark fixture preparation and catalog support.
...
This plan is in the authoring stage. No production code has been changed by this plan yet. Focused review has been completed and the plan now accounts for benchmark path resolution, writable artifact directories, strict functional-only benchmark parity, C-compatible ABI spelling, safe session ownership, atomic session cache updates, and bounded native handle reuse. The expected outcome after implementation is a feature-gated FSDB path that remains correct, has better command-scoped native resource reuse, has explicit missing/truncated/non-FSDB hardening coverage, and has reproducible FSDB benchmark commands against converted RTL artifacts.
