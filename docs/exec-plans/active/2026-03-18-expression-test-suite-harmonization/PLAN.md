...
- Decision: require `entrypoint: "logical"` and `entrypoint: "event"` negatives to declare `host_profile`, and require runtime negatives to declare `timestamp` explicitly.
  Rationale: the main ambiguity in the current negative fixtures is hidden execution context. Making host and runtime time selection explicit removes suite-local defaults from the contract and makes the manifests self-contained, even for logical or event entrypoints that are expected to fail during parsing.
  Date/Author: 2026-03-18 / OpenCode
...
2. Add the shared contract test and helper interfaces first.
   ...
   The negative manifest contract should have one root shape:
    ...
    `entrypoint: "parse"` negative cases may omit `host_profile`, `signals`, and `timestamp`. Any `entrypoint: "logical"` or `entrypoint: "event"` negative case must make its execution context explicit even when the expected diagnostic layer is `parse`. Use a stable named `host_profile` such as `event_runtime_baseline`, `integral_boolean_baseline`, or `rich_types_baseline` when a shared default host is intended, and use `host_profile: "custom"` plus inline `signals` when a case needs its own host data. Any case that is expected to fail at runtime must provide an explicit `timestamp`; the shared helper should reject runtime-negative rows that omit it. Shared manifests should also reject `entrypoint: "event"` with `layer: "runtime"` until the schema grows explicit event-evaluation frame support. Do not keep suite-local hidden defaults that are only discoverable by reading Rust setup code.
...
In `tests/common/expr_cases.rs`, define a shared positive-manifest API and a shared negative-manifest API. The positive API should be a tagged union with stable case kinds that cover parser-positive, logical-evaluation, and event-evaluation scenarios. The negative API should be one struct shape with `entrypoint`, `source`, `layer`, `code`, `span`, optional `snapshot`, optional `host_profile`, optional `signals`, and optional `timestamp`, with the rules that `entrypoint: "logical"` and `entrypoint: "event"` negatives must make their host context explicit through a named profile or `custom` inline signals, `entrypoint: "parse"` negatives must omit host context, runtime-negative cases must provide an explicit `timestamp`, and `entrypoint: "event"` plus `layer: "runtime"` must be rejected until the schema grows explicit event-frame support. That file should also expose one helper that checks `DiagnosticLayer`, diagnostic code, span, and the optional snapshot so suite files do not repeat that logic, plus one contract-test helper that rejects representative legacy shapes and runtime negatives that omit `timestamp`.
