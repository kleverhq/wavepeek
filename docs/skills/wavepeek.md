...
- `extract generic` event-row extraction: `wavepeek docs show commands/extract-generic`
...
- Event/transaction rows, handshakes, beats, and counts with payload values: `extract generic`.
...
Do not mix `--scope top.cpu` with references like `top.cpu.clk` in the same `value`, `change`, `property`, or `extract generic` query. If name lookup fails, run `scope`, then `signal --scope <SCOPE>`, then rewrite the query in one naming mode.
...
Edge-triggered `change` and `property` queries default to pre-edge value sampling, and `extract generic` always uses pre-edge sampling: the row `time` is the clock edge, while `sample_time` is where values were evaluated, printed, or extracted. Use `sample_time` for follow-up payload sampling unless you intentionally want same-edge dump values.
...
Use `extract generic` on a clocked predicate when payload values are needed:
...
- `change --signals valid,ready,payload --on "posedge clk"` can be a useful inspection shortcut, but validate event counts or transfer rows with `extract generic`.
...
- For event/transaction counts, derive counts from `extract generic` rows or `property --capture match`, not from payload transitions.
...
If results are too large, narrow the scope, filter signal names, reduce the time window, use `extract generic --max`, or use `property` to compute timestamp-only predicates before sampling payloads.
