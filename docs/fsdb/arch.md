...
> Temporary development collateral: `docs/fsdb/` exists only for the current FSDB support development branch. After FSDB support is merged, this content should either be removed or folded into the canonical documentation structure.
...
The existing VCD/FST suite must remain stable. The first refactor commit should be behavior-preserving and pass `make ci` before the FSDB backend is added.
...
- Run `make ci`; output snapshots/schema should not change.

### M2: FSDB-disabled UX

Completed implementation record: `docs/exec-plans/completed/2026-05-21-fsdb-feature-required-error/PLAN.md`.
