...
Milestones 1 through 6 are implemented. The current docs metadata contract now uses `description` in authored Markdown, current CLI JSON, export manifests, schema, docs, and tests, with `summary` retained as a legacy front matter input alias. The devcontainer source now installs MkDocs and mike from root `requirements-docs.txt` into the shared image and verifies both in `just dev-setup`. The new MkDocs preparation helper exports the embedded docs, stages `intro` as `index.md`, normalizes legacy `summary` front matter to `description` only in generated staging files, generates nav from the export manifest, and builds successfully with MkDocs strict mode. The publication wrapper can check current or tag source, create root raw artifacts, stage a mike deploy, and verify a staged bundle before pushing `gh-pages`. Just recipes expose build, serve, check, staged deploy, push-staged, and combined deploy flows, and docs-site checks now run in both local and CI quality gates. GitHub Actions now has a manual docs publication workflow with split staging and push jobs, and the release workflow dispatches docs publication only after creating the GitHub Release. Validation passed for `cargo test -q docs`, `cargo test -q --test docs_cli`, `just check-schema`, JSON parsing of devcontainer configs, `just format-justfile-check`, `git diff --check`, `mkdocs --version`, `mike --version`, `PATH="$PWD/tmp/docs-tooling-venv/bin:$PATH" just dev-setup`, `cargo run --quiet -- docs export tmp/docs-site/export --force`, `python3 -B tools/docs/prepare_mkdocs.py tmp/docs-site/export --output tmp/docs-site/mkdocs-src --config-output tmp/docs-site/mkdocs.yml --version 0.5.0 --force`, `mkdocs build --strict --config-file tmp/docs-site/mkdocs.yml`, `python3 -B -m unittest discover -s tools/docs -p "test_*.py"`, `python3 -m py_compile tools/docs/prepare_mkdocs.py tools/docs/publish_docs.py tools/docs/test_prepare_mkdocs.py tools/docs/test_publish_docs.py`, `python3 -B tools/docs/publish_docs.py check --version 0.5.0 --source-root . --work-dir tmp/docs-site-current`, `python3 -B tools/docs/publish_docs.py check --version 0.5.0 --source-ref v0.5.0 --work-dir tmp/docs-site-v050`, `just --unstable --fmt --check`, `just --summary`, `just docs-site-build`, `just docs-site-check`, `just test-aux`, `actionlint .github/workflows/*.yml`, and `just check-actions`. The overall plan remains in progress; after implementation, this section must summarize which full-gate commands passed, whether the first `0.5.0` bootstrap path was dry-run locally, and any remaining GitHub repository settings needed for Pages.
