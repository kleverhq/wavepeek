...
- `item` records carry the same row payload shape used inside `--json` data arrays for array-producing commands, the event row shape for `extract apb`, the transfer row shape for `extract axi`, or the `info` data object for `info`.
...
`--json` and `--jsonl` are mutually exclusive. `--jsonl` is available only on waveform-inspection commands: `info`, `scope`, `signal`, `value`, `change`, `property`, `extract apb`, `extract axi`, and `extract generic`.
...
`wavepeek schema --input` prints the canonical JSON input document schema snapshot, `schema/input.json`. Current input document kinds are `extract.generic.sources`, used by `wavepeek extract generic --source`; `extract.apb.source`, used by `wavepeek extract apb --source`; and `extract.axi.source`, used by `wavepeek extract axi --source`.
