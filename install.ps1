...
$app_name = 'wavepeek'
$app_version = '1.0.1'
if ($env:WAVEPEEK_DOWNLOAD_URL) {
  $ArtifactDownloadUrls = @($env:WAVEPEEK_DOWNLOAD_URL)
} elseif ($env:INSTALLER_DOWNLOAD_URL) {
  $ArtifactDownloadUrls = @($env:INSTALLER_DOWNLOAD_URL)
} elseif ($env:WAVEPEEK_INSTALLER_GHE_BASE_URL) {
  $installer_base_url = $env:WAVEPEEK_INSTALLER_GHE_BASE_URL
  $ArtifactDownloadUrls = @("$installer_base_url/kleverhq/wavepeek/releases/download/v1.0.1")
} elseif ($env:WAVEPEEK_INSTALLER_GITHUB_BASE_URL) {
  $installer_base_url = $env:WAVEPEEK_INSTALLER_GITHUB_BASE_URL
  $ArtifactDownloadUrls = @("$installer_base_url/kleverhq/wavepeek/releases/download/v1.0.1")
} else {
  $ArtifactDownloadUrls = @("https://github.com/kleverhq/wavepeek/releases/download/v1.0.1")
}

$auth_token = $env:WAVEPEEK_GITHUB_TOKEN
...
if ($env:XDG_CONFIG_HOME) {
  $receipt_home = "${env:XDG_CONFIG_HOME}\wavepeek"
} else {
  $receipt_home = "${env:LOCALAPPDATA}\wavepeek"
}

if ($env:WAVEPEEK_DISABLE_UPDATE) {
...
if ($NoModifyPath) {
    Write-Information "-NoModifyPath has been deprecated; please set WAVEPEEK_NO_MODIFY_PATH=1 in the environment"
}

if ($env:WAVEPEEK_NO_MODIFY_PATH) {
    $NoModifyPath = $true
}

$unmanaged_install = $env:WAVEPEEK_UNMANAGED_INSTALL
...
function Install-Binary($install_args) {
  ...
  $platforms = @{
    "aarch64-pc-windows-msvc" = @{
      "artifact_name" = "wavepeek-x86_64-pc-windows-msvc.zip"
      "bins" = @("wavepeek.exe")
      "libs" = @()
      "staticlibs" = @()
      "zip_ext" = ".zip"
...
function Download($download_url, $platforms, $arch) {
  ...
  if (($null -ne $info["updater"]) -and $install_updater) {
    ...
    $out_name = "$tmp\wavepeek-update.exe"
...
function Invoke-Installer($artifacts, $platforms) {
  ...
  if (($env:WAVEPEEK_INSTALL_DIR)) {
    $force_install_dir = $env:WAVEPEEK_INSTALL_DIR
    $install_layout = "flat"
  ...
  if ($install_updater) {
    ...
    [IO.File]::WriteAllLines("$receipt_home/wavepeek-receipt.json", "$receipt", $Utf8NoBomEncoding)
  }
