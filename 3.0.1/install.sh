...
APP_NAME="wavepeek"
APP_VERSION="3.0.1"
if [ -n "${WAVEPEEK_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$WAVEPEEK_DOWNLOAD_URL"
elif [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$INSTALLER_DOWNLOAD_URL"
elif [ -n "${WAVEPEEK_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$WAVEPEEK_INSTALLER_GHE_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/kleverhq/wavepeek/releases/download/v3.0.1"
elif [ -n "${WAVEPEEK_INSTALLER_GITHUB_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$WAVEPEEK_INSTALLER_GITHUB_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/kleverhq/wavepeek/releases/download/v3.0.1"
else
    ARTIFACT_DOWNLOAD_URLS="https://github.com/kleverhq/wavepeek/releases/download/v3.0.1"
fi
if [ -n "${WAVEPEEK_PRINT_VERBOSE:-}" ]; then
    PRINT_VERBOSE="$WAVEPEEK_PRINT_VERBOSE"
else
    PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
fi
if [ -n "${WAVEPEEK_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$WAVEPEEK_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi
if [ -n "${WAVEPEEK_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$WAVEPEEK_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${WAVEPEEK_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${WAVEPEEK_UNMANAGED_INSTALL:-}"
if [ -n "${UNMANAGED_INSTALL}" ]; then
    NO_MODIFY_PATH=1
    INSTALL_UPDATER=0
fi
AUTH_TOKEN="${WAVEPEEK_GITHUB_TOKEN:-}"

read -r RECEIPT <<EORECEIPT
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.32.0"},"source":{"app_name":"wavepeek","name":"wavepeek","owner":"kleverhq","release_type":"github"},"version":"3.0.1"}
EORECEIPT
...
if is_windows_posix && [ -n "${LOCALAPPDATA:-}" ]; then
    RECEIPT_HOME="$LOCALAPPDATA/wavepeek"
else
    RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/wavepeek"
fi
...
The installer for wavepeek 3.0.1

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/kleverhq/wavepeek/releases/download/v3.0.1
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set WAVEPEEK_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in 
        "wavepeek-aarch64-apple-darwin.tar.gz")
            ...
            _checksum_value="02d5472488146e7d8b63dd207e1c1004603cdfa7d15e677de7c5982f8669ed97"
            _bins="wavepeek"
            _bins_js_array='"wavepeek"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
        ...
        "wavepeek-aarch64-unknown-linux-gnu.tar.gz")
            ...
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="f4a3c74f029198da2ff4e05b83b0fc80551227292cad72a2e2cccbe1468a1bb6"
            _bins="wavepeek"
            _bins_js_array='"wavepeek"'
            _libs=""
            _libs_js_array=""
        ...
        "wavepeek-x86_64-apple-darwin.tar.gz")
            ...
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="4d1e04155682ae084655cc118c6dab4b5d00868589b8a7c6dd79273c69b28911"
            _bins="wavepeek"
            _bins_js_array='"wavepeek"'
            _libs=""
            _libs_js_array=""
        ...
        "wavepeek-x86_64-pc-windows-msvc.zip")
            ...
            _zip_ext=".zip"
            _checksum_style="sha256"
            _checksum_value="3d7e0bee10efb61fa635bab826fe51ac110b752595b7b9e0b5c72ee15c8c45b2"
            _bins="wavepeek.exe"
            _bins_js_array='"wavepeek.exe"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
        ...
        "wavepeek-x86_64-unknown-linux-gnu.tar.gz")
            ...
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="ac8f13c443cd6231b221a7d349cb1dd2bdd69ba863d34a2c365110548f51f461"
            _bins="wavepeek"
            _bins_js_array='"wavepeek"'
            _libs=""
            _libs_js_array=""
            ...
            _updater_bin=""
            ;;
        "wavepeek-x86_64-unknown-linux-musl.tar.gz")
            _arch="x86_64-unknown-linux-musl-static"
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="fa629839a19143f5b952e931687f8e98b488cabd1e978a242f5099f8df5f9199"
            _bins="wavepeek"
            _bins_js_array='"wavepeek"'
            _libs=""
            _libs_js_array=""
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="wavepeek-aarch64-apple-darwin.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="wavepeek-x86_64-apple-darwin.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-pc-windows-msvc")
            _archive="wavepeek-x86_64-pc-windows-msvc.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-unknown-linux-gnu")
            _archive="wavepeek-aarch64-unknown-linux-gnu.tar.gz"
        ...
        "x86_64-pc-windows-msvc")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="wavepeek-x86_64-unknown-linux-gnu.tar.gz"
            if ! check_glibc "2" "35"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
            ...
            _archive="wavepeek-x86_64-unknown-linux-musl.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
...
install() {
    ...
    if [ -n "${WAVEPEEK_INSTALL_DIR:-}" ]; then
        _force_install_dir="$WAVEPEEK_INSTALL_DIR"
