...
- Decision: Functional parity is strict deep equality on Wavepeek JSON fields `data` and `warnings`.
  Rationale: User requirement is full identity; permissive compare can hide real behavior drift.
  Date/Author: 2026-02-28 / OpenCode
...
Functional parity in this plan means comparing only Wavepeek JSON fields `data` and `warnings` between revised and golden runs for each compared test name. Other top-level fields are ignored unless later explicitly added to scope.
...
The warning marker to use in run report defaults to `⚠️` in this plan. If product preference differs (`❗`, `‼️`, etc.), update one constant and README legend wording.

The plan assumes warning order inside `warnings` is semantically meaningful and must match exactly. If order should be normalized, record that as a decision before implementation.
...
Milestone 4 adds functional parity checks and surfacing. During `run --compare`, compare revised vs baseline Wavepeek payload (`data`, `warnings`) per test and include a dedicated status column in README (`✅` for match, `⚠️` for mismatch, `?` for missing counterpart). During `compare`, print all tests with functional mismatch and return exit code `1` if any mismatch exists, even when timing checks pass.
...
Milestone 5 adds warning-only change-test inventory and collateral updates. Add a concrete subcommand `list-warning-only-change` to `bench/e2e/perf.py` that executes selected `change` benchmarks once in JSON mode (without Hyperfine loops), detects tests where `data` is empty and `warnings` is non-empty, and prints sorted names. Persist the resulting list into this plan under `Artifacts and Notes`. Update `docs/DEVELOPMENT.md` benchmark section to explain dual artifacts and new compare semantics.
...
4. Build warning-only change-test list and record it in this plan.

       WAVEPEEK_BIN=./target/release/wavepeek python3 bench/e2e/perf.py list-warning-only-change --filter '^change_'

   Expected signal of success is non-empty stdout with one test name per line plus `total=<N>`.

5. Run repository gates after code and docs are updated.
...
After Milestone 4, `run --compare` README includes a functional status marker column and legend. A known mismatch shows `⚠️` in report output. `compare` exits with non-zero status if any `data` or `warnings` mismatch is present, and stderr lists all mismatched test names.
...
3. If `compare` sees mismatched `data` or `warnings`, it exits `1` and prints all mismatched tests.
...
After Milestone 5, this ExecPlan includes a concrete list of warning-only `change` benchmark tests in `Artifacts and Notes`, and docs explain how to regenerate that list.
...
If warning-only classification is expensive on full matrix, run it with `--filter` slices and append incrementally until list is complete.
...
2. Wavepeek JSON capture and parse (new helper that executes command once and validates `data` + `warnings`).
3. Functional parity compare helper returning structured mismatch reasons per test.
