...
class CheckCoverageCliTest(unittest.TestCase):
    ...
    def test_fails_when_summary_type_is_unsupported(self) -> None:
        result = self.run_script({"type": "wrong", "version": "2.0.1", "data": [{"files": []}]})

        self.assertEqual(result.returncode, 1)
        self.assertIn("unsupported coverage JSON type", result.stderr)

    def test_fails_when_summary_version_is_unsupported(self) -> None:
        result = self.run_script({"type": "llvm.coverage.json.export", "version": "1.0.0", "data": [{"files": []}]})

        self.assertEqual(result.returncode, 1)
        self.assertIn("unsupported coverage JSON version", result.stderr)
        self.assertIn("expected 2.x or 3.x export schema", result.stderr)

    def test_fails_when_summary_shape_is_malformed(self) -> None:
        result = self.run_script({"type": "llvm.coverage.json.export", "version": "2.0.1", "data": [{"totals": {}}]})
