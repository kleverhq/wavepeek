...
All implementation milestones are complete. The current docs metadata contract now uses `description` in authored Markdown, current CLI JSON, export manifests, schema, docs, and tests, with `summary` retained as a legacy front matter input alias. Envelope `$schema` URLs now point to the GitHub Pages schema artifact path such as `https://kleverhq.github.io/wavepeek/wavepeek_v0.json`. The devcontainer source installs MkDocs and mike from root `requirements-docs.txt` into the shared image and verifies both in `just dev-setup`. The MkDocs preparation helper exports embedded docs, stages `intro` as `index.md`, normalizes legacy `summary` front matter to `description` only in generated staging files, generates nav from the export manifest, and builds in MkDocs strict mode. The publication wrapper checks current or release-tag source, creates root schema artifacts only, stages a mike deploy, bundles `gh-pages`, and verifies the staged bundle before a non-forced push. Push-side verification recomputes trusted path rules, checks root schema artifacts as blobs, rejects malformed `versions.json`, and preserves unrelated versions. The packaged skill is not published to Pages; `wavepeek skill` remains the supported way to get a skill matched to the installed CLI. Just recipes expose build, serve, check, staged deploy, push-staged, and combined deploy flows; docs-site checks run in both local and CI quality gates. GitHub Actions now has a manual docs publication workflow with split staging and push jobs, and the release workflow dispatches docs publication only after creating the GitHub Release. Reviews ran in code, workflow/release, docs, control, and targeted recheck lanes; the final targeted recheck reported no substantive findings before the schema URL and skill publication policy change. Validation passed for all focused commands recorded in Progress, plus final `just check` and `just ci` with `PATH="$PWD/tmp/docs-tooling-venv/bin:$PATH"` before the policy change. The historical `v0.5.0` bootstrap was dry-run locally with `python3 -B tools/docs/publish_docs.py check --version 0.5.0 --source-ref v0.5.0 --work-dir tmp/docs-site-v050` and now prepares only `wavepeek_v0.json` as a root artifact. Remaining work outside the branch is repository configuration and execution: after merge, confirm GitHub Pages publishes from the `gh-pages` branch root, then use `.github/workflows/docs.yml` manual dispatch for the initial `0.5.0` bootstrap or wait for future releases to trigger docs publication through `.github/workflows/release.yml`.
