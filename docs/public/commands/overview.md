...
Use `extract generic` when you want one row per matching synchronous event with ordered payload values sampled at the pre-edge point. It is the protocol-neutral extractor for handshakes, FIFO pushes and pops, and other transfer-like rows.
