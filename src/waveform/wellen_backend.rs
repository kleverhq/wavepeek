//! Wellen-backed waveform adapter implementation.
...
impl WellenBackend {
    ...
    #[inline]
    pub fn indexed_timestamps(&self) -> &[u64] {
        self.inner.time_table()
    }

    pub fn previous_sample_time(&self, raw_time: u64) -> Option<u64> {
        let time_table = self.inner.time_table();
        match time_table.binary_search(&raw_time) {
            Ok(0) | Err(0) => None,
            Ok(index) => Some(time_table[index - 1]),
            Err(index) => Some(time_table[index - 1]),
        }
    }
    ...
    pub fn sample_resolved_optional(
        ...
        let signal_refs = resolved
            .iter()
            .map(|signal| trusted_signal_ref(signal.id))
            .collect::<Vec<_>>();
        self.ensure_signals_loaded(&signal_refs);

        let mut sampled = Vec::with_capacity(resolved.len());
        for (signal, signal_ref) in resolved.iter().zip(signal_refs.iter().copied()) {
            sampled.push(self.decode_loaded_signal_at_index(signal, signal_ref, time_table_idx)?);
        }
    ...
    pub(crate) fn sample_expr_value(
        ...
        let signal_ref = trusted_signal_ref(resolved.id);
        self.ensure_signals_loaded(&[signal_ref]);
        let loaded = self.inner.get_signal(signal_ref).ok_or_else(|| {
    ...
    #[inline]
    pub fn indexed_signal_offset_at(
        &self,
        id: SignalId,
        time_table_idx: u32,
    ) -> Option<SignalOffsetData> {
        let signal_ref = trusted_signal_ref(id);
    ...
    pub fn decode_indexed_signal_at(
        ...
        self.decode_loaded_signal_at_index(
            resolved,
            trusted_signal_ref(resolved.id),
            time_table_idx,
        )
    }

    fn decode_loaded_signal_at_index(
        &self,
        resolved: &ResolvedSignal,
    ...
    pub fn collect_change_times_with_mode(
        ...
        let signal_refs = resolved
            .iter()
            .map(|signal| signal_ref(signal.id))
        ...
        for signal in resolved {
            let signal_ref = signal_ref(signal.id)?;
            let loaded = self.inner.get_signal(signal_ref).ok_or_else(|| {
                WavepeekError::Internal(format!(
                    "signal '{}' could not be loaded from waveform backend",
                    signal.path
    ...
    pub(crate) fn collect_expr_candidate_times_with_mode(
        ...
        if !event_sources.is_empty() {
            ...
            let signal_refs = event_sources
                ...
                .map(|signal| signal_ref(signal.id))
                .collect::<Vec<_>>();
            let signal_refs = signal_refs.into_iter().collect::<Result<Vec<_>, _>>()?;
...
#[inline]
fn trusted_signal_ref(id: SignalId) -> SignalRef {
    debug_assert!(
        id.as_u64() <= u64::from(u32::MAX - 1),
        "backend-generated SignalId must stay inside Wellen SignalRef range"
    );
    SignalRef::from_index(id.as_u64() as usize)
        .expect("backend-generated SignalId must represent a Wellen SignalRef")
}
...
mod tests {
    ...
    use super::{scope_type_alias, should_use_multi_thread_signal_load, var_type_alias};
    use crate::waveform::{
    ...
    fn previous_sample_time_returns_strict_predecessor() {
        ...
        assert_eq!(waveform.previous_sample_time(5), Some(0));
        assert_eq!(waveform.previous_sample_time(7), Some(5));
        assert_eq!(waveform.previous_sample_time(10), Some(5));
    }
