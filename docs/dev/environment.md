...
FSDB work is optional and local-only unless a task explicitly says otherwise. The devcontainer sets `VERDI_HOME=/opt/verdi`, which is normally a host-mounted cache location. `tools/codex/check_fsdb_env.py` verifies that the mount contains a usable Synopsys Verdi FSDB Reader SDK before any FSDB build, test, fixture, or benchmark recipe runs.
